...
*/		if(!this.$el.is("select")){
			if(settings.queryWidget){
				$.extend(settings, {
					ajax: this.defaults.ajaxSetup(settings)
				});
			}
			$.extend(settings, {
				initSelection : function(element, callback){
					this.defaults.initSelection(settings, element, callback);
				}.bind(this),
				formatSelection: this.defaults.format,
				formatResult: this.defaults.format
			});
		}
