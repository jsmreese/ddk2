...
xReq =null;
  ...
  _error: function(message) {
	...
	function run(objID, metricname, keywords, callback, options) {
        ...
        if (typeof callback === "function") {
            ...
            load(objID, metricname, keywords, _callback, options);
        } else {
            load(objID, metricname, keywords, callback, options);
        }
    }

    function load(objID, metricname, keywords, callback, options){
		...
		callback = _.wrap(callback || returnContents, function (func, data, header, id) {
			...
			if (_options && _options.requestState) {
				...
				// resolve deferred object if it is "pending"
				if (requestState = "pending") {
					_options.requestState.resolve();
				}
			}
			
			// if deferred state is "rejected"
			// return without executing callback
			if (requestState === "rejected") {
				DDK.warn("Request state is 'rejected'. Ignoring AMEngine response." + ((_options && _options.requestId) ? "Request id: " + _options.requestId : ""));
				return;
			}
			
			// execute callback for "pending" or "resolved" states
			func(data, header, id);
		});
        ProcessURL3(objID, metricname, keywords, callback, _options);
    }
	...
	function ProcessURL3(objID, metricname, keywords, callback, options)
	{
		...
		if (_.isPlainObject(keywords)) {
			keywords = _.transform(keywords, function (accumulator, value, key) {
