	...
	navset2Element: function (accumulator, elem, index) {
		var addDataNav = function(elemType, config){
			...
			"checkbox": function(options){
				...
				if(options.elemText){
					html += "<label for=\"" + options.elemId + "\">" + options.elemText + "</label>";
				}
				return html;
			},
			"radio": function(options){
				var html = "<input id=\"" + options.elemId + "\" type=\"radio\" class=\"is-plain-button " + options.elemClassName + "\" " + options.elemAttr + "></input>";
				if(options.elemText){
					html += "<label for=\"" + options.elemId + "\">" + options.elemText + "</label>";
				}
				return html;
		...
		accumulator += createElem(elem.elemType, elem);
		/*
